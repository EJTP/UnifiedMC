	...
	{#snippet children({ thumbItems })}
		...
		{#each thumbItems as thumb (thumb.index)}
			<SliderPrimitive.Thumb
				...
				index={thumb.index}
				class="relative size-3 rounded-full border border-ring bg-foreground ring-ring/50 transition-[color,box-shadow] after:absolute after:inset-x-0 after:-inset-y-2 hover:ring-3 focus-visible:ring-3 focus-visible:outline-hidden active:ring-3 block shrink-0 select-none disabled:pointer-events-none disabled:opacity-50"
			/>
		{/each}
	{/snippet}
</SliderPrimitive.Root>
